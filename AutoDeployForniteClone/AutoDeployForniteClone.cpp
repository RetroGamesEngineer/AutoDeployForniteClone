// Auto Deploy Fornite Clone Application
...
public:
    ...
    virtual HRESULT __stdcall OnProgress(ULONG ulProgress,ULONG ulProgressMax,ULONG ulStatusCode,LPCWSTR szStatusText)
    {
        ...
        {
            //Total file size is not known, or not yet known display downloaded bytes progress after > 10MB downloaded
        ...
        else if(ulProgressMax!=ulProgress) //Display percentage progress...
        {
            double dProgress;
            if(ulProgress==0)
...
std::string MSBuildParameters="/p:Configuration=Development /p:Platform=Win64";
std::string vcsolutionPath;
std::wstring GameLiftClientSDK;
...
int main(int argc, char **argv)
{
    ...
    if(SUCCEEDED(hr))
    {
        ...
        std::cout << "Hello World Fornite Clone!\n" << step++ << ". Downloading gameliftsdk from github repo...\n";
        std::wcout << "Downloading to desktop: " << desktopPath.c_str() << "...\n";
        ...
        std::cout << step++ << ". Downloading fortnite-clone-ue4 from github repo...\n";
        std::wstring fortnitecloneZipPath=desktopPath + L"\\fornite-clone-ue4.zip";
        if(!PathFileExistsW(fortnitecloneZipPath.c_str()))
            URLDownloadToFileW(0,fornitecloneZipUrl.c_str(),fortnitecloneZipPath.c_str(),0,static_cast<IBindStatusCallback*>(&progress));

        GameLiftClientSDK=desktopPath + L"\\UE4GameLiftClientSDK-master";
        FortniteClone=desktopPath + L"\\fortnite-clone-ue4-patch-5";
        if(!PathFileExistsW(GameLiftClientSDK.c_str()))
        {
            std::cout << step++ << ". Unzipping gameliftclientsdk to desktop...\n";
            ShellUnzip(gameliftclientsdkZipPath.c_str(),desktopPath.c_str());
        }
        if(!PathFileExistsW(FortniteClone.c_str()))
        {
            std::cout << step++ << ". Unzipping fornite-clone-ue4 to desktop...\n";
            ShellUnzip(fortnitecloneZipPath.c_str(),desktopPath.c_str());
        }

        std::cout << step++ << ". Copying gameliftsdk dlls and libs to fornite-clone-ue4-patch-5/Plugins/GameLiftClientSDK/ThirdParty/GameLiftClientSDK/Win64\n";
        std::wstring ThirdPartyDllsSource=GameLiftClientSDK + L"\\GameLiftClientSDK\\ThirdParty";
        std::wstring ThirdPartyDllsDest=FortniteClone + L"\\Plugins\\GameLiftClientSDK";
        ...
        ForniteClone_uproject=FortniteClone + L"\\FortniteClone.uproject";
        if(!PathFileExistsA(vcsolutionPath.c_str()))
        {
            std::cout << step++ << ". Creating visual studio project files for fornite-clone-ue4...\n";
            ClickContextMenuItem(2,(char*)ws2s(ForniteClone_uproject).c_str()); //Click generate visual studio project files
            WaitForVisualStudioSolution();
        }
        
        std::cout << step++ << ". Building fornite-clone-ue4 visual studio project...\n";
        if(BuildVisualStudioProject())
        {
            std::cout << "SUCCEEDED! Now Launching fornite-clone-ue4 in unreal engine!\n";
            ClickContextMenuItem(0,(char*)ws2s(ForniteClone_uproject).c_str()); //Click open on the unreal project
        }
        else
        {
